...
install.packages('SnowballC') # 어근 처리 함수 : 동명사, 분사 ->명사
library(SnowballC) 
...
inspect(sms_corpus[4])
sms_corpus = tm_map(sms_corpus, removeWords, stopwords("SMART"), '짙') # 5) stopwords(the, of, and 등) 제거  
inspect(sms_corpus[4])
...
# Non-/sparse entries: 33887/37560425           >>  실제로 데이터가 들어간 셀의 비율이 이렇게 적음
# Sparsity           : 100%                     >>  희소비율 
# Maximal term length: 40                       >>  가장 긴 단어 길이
# Weighting          : term frequency (tf)      >>  단어 출연 빈도?에따라 ?
...
# 행 단위(단어수) 합계 -> 내림차순 정렬   : 출현빈도수 볼 수 있다.
...
# Sparsity           : 100%
# Maximal term length: 8          >> control = list(wordLengths= c(1,8)로 줄었음.
...
TF <- 2  # 한 문장 2회 출현
DF <- 10  ## 전체에서 출현한 빈도수가 많을수록 TfIdf는 작아진다.
...
# 단어 길이 검색 
class(sms_dtm1)  # "DocumentTermMatrix"    "simple_triplet_matrix"
str(sms_dtm1$dimnames)
# List of 2
# $ Docs : chr [1:5558] "1" "2" "3" "4" ...
# $ Terms: chr [1:6423] "a" "are" "check" "good" ...
str(sms_dtm1)
...
# 단어 출현빈도수 검색
word_freq <- findFreqTerms(sms_dtm1, lowfreq = 40)  # tm 소속
word_freq
...
# 1. sms_spam.csv 가져오기 - stringsAsFactors = FALSE : factor형으로 읽지 않음 
setwd("D:/ITWILL/Work/2_Rwork/Part-IV/data")
sms_data <- read.csv('sms_spam.csv', stringsAsFactors = FALSE)
str(sms_data)
...
# X          열단위로   이 함수에 들어감.
dim(sms_dtm_mat_test) 
# 5558 6122

sms_dtm_mat_text[,1] # 첫번째 단어  
table(sms_dtm_mat_text[,1])  # sms_dtm_mat_text[,1]
...
# 6. csv file save
write.csv(sms_dtm_df, "C:/ITWILL/Work/2_Rwork/Part-IV/sms_dtm_df.csv",
          quote=F, row.names=F)
