...
#           (맥주&고기/tot트렌젝션) (맥주&고기/맥주)  (신뢰도/고기(후행사건))
# 라면,우유->맥주       1/6=0.166       1/3=0.333    0.333/(2/6) = 0.99~1
...
# 3. rule 발견(생성) - 지지도,신뢰도 = 0.1, 0.8default
# apriori(트랜잭션 data, parameter=list(supp, conf))
apriori(tran, parameter=list(supp=0.166, con=0.3333))  # 30 rule(s)
...
rule<- apriori(tran, parameter = list(supp=0.1, conf=0.1)) # 35 rule 
                                # 지지도 >= 0.1, 신뢰도>=0.1
inspect(rule) # 규칙 보기
# [32] {라면,우유} => {맥주} 0.1666667 0.3333333  1.000 1   앞에서 직접계산한 것과 똑같음.
...
# [1] {}          => {우유} 0.8333333 0.8333333  1.0  5    
# [2] {맥주}      => {우유} 0.3333333 1.0000000  1.2  2    
# [3] {고기,과일} => {라면} 0.1666667 1.0000000  1.5  1    
# [4] {과일,라면} => {고기} 0.1666667 1.0000000  1.5  1    
# [5] {고기,맥주} => {우유} 0.1666667 1.0000000  1.2  1    
# [6] {라면,맥주} => {우유} 0.1666667 1.0000000  1.2  1    
...
# 
# most frequent items:                              >>> 제일많이구매한.
#   10001519 10003364 10003373 10093119 10003332  (Other) 
# 186      183       91       84       71      557  >>> 구매 횟수
# 
# element (itemset/transaction) length distribution:
#   sizes 
# 1   2   3   4   5   6   7   8                     >>> 아이템 수
# 12  25  16  20 119  12  37   7                    >>> 트렌젝션에서 빈도수(tot248)
# 
...
ar1<- apriori(Adult, parameter = list(supp=0.2)) # 지지도 높임  # 1306 rule(s)
ar2<- apriori(Adult, parameter = list(supp=0.2, conf=0.95)) # 신뢰도 높임  # 348 rule(s)
ar3<- apriori(Adult, parameter = list(supp=0.3, conf=0.95)) # 신뢰도 높임  # 124 rule(s)
ar4<- apriori(Adult, parameter = list(supp=0.35, conf=0.95)) # 신뢰도 높임  # 67 rule(s)
ar5<- apriori(Adult, parameter = list(supp=0.4, conf=0.95)) # 신뢰도 높임  # 36 rule(s)
ar6<- apriori(Adult, parameter=list(supp=0.5, conf=0.5))
# 결과보기
...
rules <- apriori(Groceries, parameter=list(supp=0.001, conf=0.8))  # 410 rule(s)
...
inspect(but_yog)
# butter or yogutr 가 조회됨
plot(but_yog, meghod="graph")
