...
# 형식) naiveBayes(train(의x변수), class(훈련셋의y변수)) - train : x변수, calss : y변수
...
# 5. 분류정확도 
acc <- (tab[1,1]+tab[2,2])/length(test_w$RainTomorrow)
acc  # 0.7924085
...
#############################################
## 제2-2장 NB 연습문제 - 희소행렬 ver.
## 17-1에서 저장한 sms_spam.csv 파일로 실습
#############################################

setwd("C:/ITWILL/Work/2_Rwork/Part-IV")
sms_data <- read.csv('sms_dtm_df.csv', stringsAsFactors = FALSE)
str(sms_data)
