...
p+geom_point()  # point 추가
ggsave(file="C:/ITWILL/Work/2_Rwork/output/diamond_price.pdf") # 가장 최근 그래프 저장
ggsave(file="C:/ITWILL/Work/2_Rwork/output/diamond_price.jpg", dpi=72)
...
ggsave(file="C:/ITWILL/Work/2_Rwork/output/bar.png", plot=p, width=10, height=5)
ggsave(file="C:/ITWILL/Work/2_Rwork/output/bar.pdf", plot=p, width=10, height=5)
...
map <- get_stamenmap(seoul, zoom=7,  maptype='terrain')
...
daegu_map <- get_stamenmap(daegu, zoom=7, maptype='watercolor')
ggmap(daegu_map)

# [단계 1] dataset 가져오기
# 통계청에서 제공하는 인구수 정보

pop1 <- read.csv(file.choose())  #2019
str(pop1)
...
lat <- pop1$'LAT'
tot_pop <- pop1$"총인구수"
tot_pop <- as.numeric(str_replace_all(pop1$'총인구수', ",", ""))
tot_pop %>% head()


df <- data.frame(region, long, lat, tot_pop)
df
# 위도, 경도별 지역명과 포인터크기로 나타낸 총 인구수를 표시하고 싶다.
...
# [단계 4] 레이어 2 : 각 지역별 포인트 추가
df$lon
layer2 <- layer1 + geom_point(data = df, aes(x=long, y=lat,
                                             color=factor(tot_pop),
                                             size = factor(tot_pop)))
layer2

# [단계 5] 레이어3 : 각 지역별 포인트 옆에 지명 추가하기
layer3 <- layer2 + geom_text(data=df,aes(x=long+0.01, y=lat+0.08, label=region,
                                         size=3))
                          
layer3

# [단계 6] 저장
ggsave(file="C:/ITWILL/Work/2_Rwork/output/population.png", width=10, height=5)
